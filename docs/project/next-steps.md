...
- `routed_action_execution_envelope` now also has a durable relational contract plus committed Prisma schema, migration, and repository adapter contract artifacts
- it still has no adapter-backed relational repository, concrete Prisma adapter, or shared-bundle coverage
- runtime engines are still intentionally out of scope, so the next bounded step should stay narrow and repository-rollout-focused

## Recommended near-future sequence
1. add the adapter-backed relational repository and concrete Prisma adapter for `routed_action_execution_envelope`
2. add slice-level shared composition and shared Prisma bundle extension through `routed_action_execution_envelope`
...
- mixing shared-bundle rollout, integration expansion, or setup-mutation runtime behavior into the routed-action repository-rollout step
