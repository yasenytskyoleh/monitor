...
test("exposes expected lifecycle enums for the first product-domain slice", () => {
  ...
  assert.deepEqual(SETUP_DEFINITION_STATUSES, ["draft", "active", "paused", "archived"]);
  ...
  assert.deepEqual(SETUP_LIFECYCLE_MUTATION_RESULT_STATUSES, [
    "applied",
    "rejected_validation",
    "rejected_lifecycle",
    "failed"
  ]);
  assert.deepEqual(SETUP_REFINEMENT_STATUSES, [
    "proposed",
    "accepted",
    "in_progress",
    "completed",
  ...
  assert.deepEqual(SETUP_REFINEMENT_REQUEST_RESULT_STATUSES, [
    "created",
    "rejected_validation",
    "rejected_lifecycle",
    "failed"
  ...
  assert.deepEqual(SETUP_REVISION_ACTIVATION_OUTCOMES, [
    "activated",
    "superseded_previous",
    "already_active",
    "rejected"
  ]);
  assert.deepEqual(SETUP_REVISION_ACTIVATION_RESULT_STATUSES, [
    "activated",
    "superseded_previous",
    "already_active",
    "rejected",
  ...
  assert.deepEqual(SETUP_REVISION_HISTORY_QUERY_MODES, [
    "family_history",
    "single_revision",
    "family_comparison"
  ]);
  assert.deepEqual(REVISION_HISTORY_QUERY_STATUSES, [
    "resolved",
    "rejected",
    "failed"
  ]);
  assert.deepEqual(SETUP_REVISION_COMPARISON_STATUSES, [
    "compared",
    "insufficient_evidence"
  ]);
  assert.deepEqual(REVISION_COMPARISON_RESULT_STATUSES, [
    "compared",
    "insufficient_evidence",
    "rejected",
    "failed"
  ]);
  assert.deepEqual(REVISION_IMPACT_CLASSIFICATIONS, [
    "improved",
    "degraded",
    "mixed",
    "inconclusive"
  ]);
  assert.deepEqual(REVISION_EVIDENCE_SUFFICIENCY_LEVELS, [
    "sufficient",
    "limited",
    "insufficient"
  ]);
